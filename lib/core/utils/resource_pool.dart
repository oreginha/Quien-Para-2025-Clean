...
class ResourcePool<T> {
  ...
  ResourcePool({
    ...
    _cleanupTimer = Timer.periodic(_idleTimeout, (_) => _cleanupIdleResources());
  }
  ...
  Future<void> release(T resource) async {
    ...
    if (kDebugMode) {
      ...
      logger.d('ResourcePool: $availableCount disponibles, $inUseCount en uso, $waitingCount esperando');
    }
  }
  ...
  Future<void> _cleanupIdleResources() async {
    ...
    if (idleResources.isNotEmpty && kDebugMode) {
      logger.d('ResourcePool: Limpiados ${idleResources.length} recursos inactivos');
    }
  }
