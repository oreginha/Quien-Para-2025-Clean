...
class PerformanceMetrics {
  ...
  void recordMetric(String category, String name, dynamic value) {
    ...
    if (value is num) {
      ...
      _metrics[category]!['${name}_avg'] = 
          _metrics[category]!['${name}_total'] / _metrics[category]!['${name}_count'];
    }
  }
  ...
  Map<String, dynamic> getComparison(String category, {
    ...
    for (final metric in allMetrics) {
      ...
      if (beforeValues.isNotEmpty && afterValues.isNotEmpty) {
        final beforeAvg = beforeValues.reduce((a, b) => a + b) / beforeValues.length;
        final afterAvg = afterValues.reduce((a, b) => a + b) / afterValues.length;
